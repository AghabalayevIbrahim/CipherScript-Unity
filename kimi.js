...
// Exports the same CSProvider interface as providers/deepseek.js and gemini.js;
...
const CSProvider = (() => {
  ...
  // Scope to the SITE's composer only: skip CipherScript's own injected UI (the
  // settings textarea #cs-set-text in #cs-root). On login/OAuth pages with no
  ...
  const getEditor = () => {
    ...
    // barAnchor() then hugged the fixed CipherScript bar over the editor. The real
  ...
  // subtree - inserting our foreign #cs-bar into `.chat-editor` makes Vue's next
  ...
  // the core keeps the bar in its own #cs-root (position:fixed) but positions it
  ...
  function setInputLock(on) {
    ...
    const ed = getEditor();
    if (on) {
      if (ed) { ed.setAttribute("contenteditable", "false"); ed.setAttribute("data-cs-locked", "1"); }
      ...
      if (ed) { ed.setAttribute("contenteditable", "true"); ed.removeAttribute("data-cs-locked"); }
  ...
  // its built-in tools over the CipherScript command protocol. On K3 the only
  ...
  function nativeAgentModeOn() {
    ...
    // its own tooling instead of emitting CipherScript command blocks.
  ...
  function modeWarning() {
    if (nativeAgentModeOn())
      return `Switch the model picker off <b>K3 Swarm</b> (pick <b>K3</b> or <b>Instantané</b>) - ` +
        `Kimi's own agentic mode replaces the CipherScript commands with its native tools and breaks the agent loop.`;
    return "";
  }
  ...
  function overlayBlocking() {
    for (const mask of document.querySelectorAll(MODAL_MASK_SEL)) {
      if (mask.closest("#cs-root")) continue;
  ...
  // Pick a model that actually speaks the CipherScript command protocol, ONCE,
  ...
  // its identity) with .cs-cmd-mask so the overlay.css rule re-hides recreated
  ...
  function findToolBlockSpot(item /*, chip */) {
    ...
    md.querySelectorAll(S.codeWrap).forEach((cw) => {
      if (cw.closest(".cs-chip") || cw.closest(S.thinking)) return; // never a drafted-in-reasoning block
    ...
    [...md.children].forEach((el) => {
      if (el.classList.contains("cs-chip") || el.closest(S.codeWrap) ||
    ...
    md.querySelectorAll(".paragraph, p").forEach((el) => {
      if (el.classList.contains("cs-tool-hide") || el.closest(".cs-chip") ||
  ...
  return {
    ...
    // the capture is shown in the core's left-hand CipherScript popup (ui.showImages).
    supportsVision: true,
    timings,
    ...
    reliableCounts: true,
    // Shown as a permanent, non-intrusive notice in the CipherScript panel.
    // Kimi sometimes reaches for its OWN built-in/native tools (web search, code
    // runner, etc.) instead of emitting the CipherScript command blocks that drive
    // the Editor - model behavior, not something the prompt fully prevents.
    unstableWarning:
      "Kimi sometimes uses its own native tools instead of the Unity commands (model behavior, not the extension). " +
      "If it stops acting in the Unity Editor and answers in plain text or runs its own tools, remind it to use the CipherScript commands - or start a new session.",
