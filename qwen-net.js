...
// that carry CipherScript commands) in a Monaco editor that DISPOSES off-screen
...
// unreliable (a `{"command":...}` becomes just `{`, a 50-line execute_code becomes
...
// done flag into a DOM node (#cs-qwen-net, JSON textContent) that the isolated
...
(() => {
  "use strict";
  if (window.__csQwenNet) return;
  window.__csQwenNet = true;

  const NODE_ID = "cs-qwen-net";
  const node = () => {
