...
// Drives any AI chat site through the CSProvider interface (providers/*.js):
// waits for the model's reply, parses CipherScript commands (CSParse), asks the
// background worker to execute them on the Unity MCP bridge, and feeds the
...
(() => {
  ...
  const log = (...a) => console.log("[cipherscript]", ...a);
  ...
  // console (filter "[cs-diag]") or window.__csDiag (also mirrored onto a hidden
  // DOM node for a main-world inspector). Each entry carries a turn snapshot.
  const CS_DIAG_MAX = 300;
  const _diag = [];
  function diag(event, data) {
    ...
    const e = { t: Date.now(), iso: new Date().toISOString().slice(11, 23), event,
                data: data || null, snap };
    _diag.push(e);
    if (_diag.length > CS_DIAG_MAX) _diag.shift();
    try { console.log("[cs-diag]", e.iso, event, JSON.stringify({ ...data, ...snap })); } catch {}
    try {
      let n = document.getElementById("cs-diag-log");
      if (!n) { n = document.createElement("script"); n.type = "application/json"; n.id = "cs-diag-log"; (document.body || document.documentElement).appendChild(n); }
      n.textContent = JSON.stringify(_diag);
    } catch {}
    try { window.__csDiag = _diag; } catch {}
  }
  P.init({ diag });
  ...
  // AI chat sites CipherScript works on. Keep in sync with manifest.json
  ...
  const A = {
    ...
    // execute_code that was mid-stream in an A/B turn); that restored turn looks
    ...
    // For the KNOWN vision tool (screen_capture) toolCategory already
  ...
  async function waitForResponse(base) {
    ...
    while (Date.now() - lastActiveAt < TIMEOUT) {
      ...
      // away in the Editor. Park until the tab is foreground again, then slide EVERY
      ...
      const blockActive = CSParse.hasOpenToolBlock(d.reply) && Date.now() - lastChangeAt < 6000;
      ...
      // execute_code seen live on GLM) can pause >STABLE_MS between tokens - that
      ...
      const stuckDone = started && d.reply && Date.now() - lastChangeAt > STABLE_MS &&
        !(gen && CSParse.hasOpenToolBlock(d.reply));
      ...
      const cmdShaped = P.replyUnsettled && (
        CSParse.hasToolSignature(r) ||
        (CSParse.CS_END_RE.test(r) && !CSParse.CS_START_RE.test(r)) ||
        (/"(?:edits|old_string|new_string|file_path|target_file|uri)"\s*:/.test(r) &&
          !/"command"\s*:/.test(r))
      );
      ...
      if (CSParse.hasToolSignature(r)) {
        const calls = CSParse.parseToolCalls(r);
        ...
        if (r.includes(CSParse.START_M) || CSParse.CS_START_RE.test(r)) return { kind: "parse_error", reason: "malformed", raw: r, item: d.item };
        ...
        // while it streamed).
        if (CSParse.hasOpenToolBlock(r)) {
          const saved = CSParse.salvageCutOff(r);
          if (saved) {
        ...
        // mid-code). Unlike execute_code there is NO ###CSHARP### fallback, so the
        ...
        const nm = CSParse.toolNameFromText(r);
      ...
      // Malformed execute_code: the model wrote the ###END_C#### closer but
      // FORGOT the ###C#### opener, so hasToolSignature missed it and the block
      ...
      if (CSParse.CS_END_RE.test(r) && !CSParse.CS_START_RE.test(r) && !r.includes(CSParse.START_M)) {
        return { kind: "parse_error", reason: "csOpener", raw: r, item: d.item };
      }
      // Malformed command: the model emitted a tool's RAW ARGUMENTS as a bare JSON
      // object (e.g. {"edits":[...],"file_path":...,"uri":...}) instead of
      ...
      if (/"(?:edits|old_string|new_string|file_path|target_file|uri)"\s*:/.test(r) &&
          !/"command"\s*:/.test(r)) {
  ...
  // The known vision tool (screen_capture) is themed "screen" by name via
  // CS.toolCategory. A custom MCP tool's NAME reveals nothing, so we learn which
  ...
  function rememberImageTool(name) {
    ...
    try { chrome.storage.local.set({ csImageTools: [...A.imageTools].slice(-200) }); } catch {}
  }
  try {
    chrome.storage.local.get("csImageTools", (r) => {
      if (r && Array.isArray(r.csImageTools)) for (const n of r.csImageTools) A.imageTools.add(n);
  ...
  //
  // DEGRADED MODE (the Editor closed, running on an addon server like Blender)
  // is where this used to hurt: a list_tools whose Unity half is dead blocks the
  ...
  async function runTool(call) {
    ...
    if (!name) return CS.FEEDBACK.parseError("malformed");
    ...
    // still ran execute_code). Parking here (no time cap) means the call runs the
    ...
    if (isBlockedTool(name)) {
      if (VISION_TOOLS.has(bareName)) {
        return `ERROR: '${bareName}' is unavailable here - this assistant cannot see images. Do NOT call it again. Inspect the scene programmatically instead (e.g. find_gameobjects, read_console, get_sha, manage_script).`;
      }
      return `ERROR: the '${bareName}' command timed out and is unavailable in this environment. Do NOT call it again - complete the task yourself using the other commands (execute_code, manage_gameobject, script_apply_edits, etc.).`;
    }
    // Virtual command: list the MCP server(s) CipherScript is currently connected
    ...
    if (name === "list_mcp_servers") {
      ...
      const lines = servers.length
        ? servers.map((sv) => {
            const label = sv.id === "unity" ? "Unity Editor (primary)" : `${sv.id} (addon)`;
            return `- ${sv.id}: ${label} - ${sv.alive ? `${sv.tools || 0} commands available` : "offline (no tools)"}`;
          })
        : ["- unity: Unity Editor (primary) - unknown (bridge did not report server health)"];
      return (
        `Output of 'list_mcp_servers':\n` +
        `Connected MCP servers (${lines.length}):\n${lines.join("\n")}\n` +
        `Use list_commands with a "server" param (one of the ids above) to see that server's exact commands. Without "server", list_commands defaults to "unity".`
      );
    }
    // Virtual command: list available commands with full details. Defaults to
    // the primary Unity server - a DIFFERENT server's tools only ever show up
    // if the model explicitly asks via {"server": "<id>"} (see list_mcp_servers).
    if (name === "list_commands" || name === "list_tools") {
      await ensureTools();
      const requested = (args.server || "unity").trim();
      // The MCP server keeps advertising its catalogue even with no Unity Editor
      // attached, so list_commands would hand back the full command list and read
      // as "Unity is fine" - then every command silently fails. When Unity is
      // actually unusable, short-circuit the DEFAULT (unity) listing into a plain
      // "Unity is down" note that points the model at the other server(s), so it
      // can keep working in degraded mode instead of firing dead Unity commands.
      if (requested === "unity") {
        const s = A.bridge || {};
        const srv = s.servers || [];
        const un = srv.find((x) => x.id === "unity");
        const unAlive = un ? !!un.alive : (!!s.mcpAlive || srv.some((x) => x.alive));
        const unUsable = !!s.connected && unAlive && s.unity !== false;
        if (!unUsable) {
          const others = srv.filter((x) => x.id !== "unity" && x.alive && (x.tools || 0) > 0);
          const otherStr = others.length
            ? `Other connected MCP server(s): ${others.map((x) => x.id).join(", ")}. Call list_mcp_servers, then list_commands with a "server" param to use them for anything that does not need Unity.`
            : `No other MCP server is connected right now.`;
          return `Output of '${name}':\nUnity Editor is currently OFFLINE (closed, or its MCP server is not running), so its commands cannot run. This is an environment problem on the user's machine, not your mistake. Tell the user in one short sentence to open their Unity project and start the MCP server (Window > Unity MCP Toolkit). ${otherStr}`;
        }
      }
      ...
      // hiding everything.
      const scoped = A.toolList.filter((t) => (t.server || "unity") === requested);
      if (!A.toolList.length) return `Output of '${name}':\nNo commands available - the bridge or Unity Editor may be offline.`;
      if (!scoped.length) {
        return `Output of '${name}':\nERROR: no server named "${requested}" is connected. Connected servers: ${[...known].join(", ") || "unity"}. Call list_mcp_servers to check.`;
      }
      const lines = scoped.map((t) => {
        ...
        const note = CS.TOOL_NOTES[bareToolName(t.name)];
        const noteStr = note ? `\n    ⚠ ${note}` : "";
    ...
    if (A.toolNames.size && !A.toolNames.has(name)) {
      return CS.FEEDBACK.unknownTool(name, [...A.toolNames]);
    }
    // Unity MCP Toolkit REQUIRES `action` on execute_code (enum execute / get_history /
    // replay / clear_history). The ###C#### parser already fills it in, but the
    // model may also write the JSON form without it - default to "execute" so the
    // call never soft-fails with "'action' is required".
    if (bareName === "execute_code" && !args.action) args.action = "execute";
    // execute_code compiles in-memory and runs synchronously on the Unity main
    // thread - a ~30s budget (C# compilation can take a beat longer than the old
    // C# executor's ~20s on first use).
    const timeout = name === "execute_code" ? 30000 : 120000;
    ...
    if (!r) return CS.FEEDBACK.bridgeOffline;
    // The MCP server answers SUCCESSFULLY (ok:true) when no Unity Editor is
    // attached (closed / its MCP server not running) - with an explanatory text
    // instead of a result. Surface it as a proper environment ERROR so the
    // model stops and tells the user, instead of treating it as tool output.
    if (r.ok && /not connected to an? ?(?:unity|editor)|no (?:unity )?editor|unity editor (?:is )?not (?:connected|running)|editor (?:is )?not (?:connected|running)|unity is not running|no unity instance/i.test(r.text || "")) {
      ui.banner("warn", "Unity Editor is not connected",
        "Open your Unity project and start the MCP server (Window → Unity MCP Toolkit → Start server), then try again.");
      return CS.FEEDBACK.unityOffline;
    }
    // The Unity MCP reports missing/invalid required parameters as a SUCCESS
    // whose text is just the complaint (e.g. "'action' is required").
    ...
    // Unity MCP Toolkit reports C# compile/runtime errors as normal tool errors, so
    // no stack-trace re-shaping is needed here (the execute_code branch below
    // turns them into tailored feedback).
    if (r.ok) {
      ...
      if (r.images && r.images.length) {
        // Show the capture in a left-hand CipherScript popup (from the in-memory
    ...
    if (r.kind === "disconnected") return CS.FEEDBACK.bridgeOffline;
    if (r.kind === "timeout") {
      return `ERROR: tool '${name}' timed out after ${name === "execute_code" ? 30 : 120}s.\n${r.error}\nTry a shorter/simpler call or check that the Unity Editor is open and responsive.`;
    }
    if (name === "execute_code") {
      const err = r.error || "";
      const hint = /empty|marker|syntax|compile|CS\d{4}/i.test(err)
        ? "Your code block was empty, the marker was wrong, or the C# did not compile. Use exactly ###C#### (three hashes each side) - never ###C---. The code must be between ###C#### and ###END_C#### and must be a valid method body (no class/method wrapper)."
        : /reference|does not exist|not defined|namespace/i.test(err)
          ? "You called a type or member that does not exist in this context. Check the exact API with unity_reflect or unity_docs before retrying."
          : "C# compile/runtime error. Remember: it runs as a method body, use 'return' to output values (not Debug.Log), never block the main thread, and only call APIs that exist in the current Unity Editor context.";
      return `ERROR in execute_code: ${err}\n\n${hint}\n\nFix the code and retry.`;
    }
  ...
  function callBody(call) {
    const a = call.arguments || {};
    if (call.tool === "execute_code") return (a.code || "").trim();
  ...
  async function agentLoop(base) {
    ...
    try {
      while (!A.stop) {
        ...
        //    working in the Editor) -> still "visible" -> the agent keeps
        ...
        if (res.kind === "truncated") {
          ...
          if (truncCount < MAX_TRUNC) {
            ...
            base = await submitAndGetBase(CS.FEEDBACK.truncated);
            continue;
          }
        ...
        if (res.kind === "parse_error") {
          ...
          const failName = CSParse.toolNameFromText(res.raw || "") || "command";
          if (res.item) {
            const detail = res.reason === "unclosed" ? "cut off"
              : res.reason === "csOpener" ? "missing ###C####"
              : res.reason === "envelope" ? "bad format"
              : "bad JSON";
            decorate.toolBox(res.item, failName, "err", detail, true, "", CS.toolCategory(failName));
          }
          // Pass the detected command name so the feedback only offers the
          // ###C#### block when it actually applies (execute_code) - never for a
          // truncated/broken JSON-only command.
          base = await submitAndGetBase(CS.FEEDBACK.parseError(res.reason, failName));
          continue;
        }
        ...
        if (res.kind === "tool") {
          ...
          if (calls.length > 1) {
            base = await submitAndGetBase(CS.FEEDBACK.multiTool(calls.map((c) => c.tool || "?")));
            continue;
          }
          const call = calls[0];
          ...
          const category = learnedImg ? "screen" : CS.toolCategory(call.tool);
          ...
          //     elapsedOn(csToolT0) counts the parked time, so a 20-min minimize
          ...
          // A tool (Unity OR any custom MCP server) that actually RETURNED an
          ...
          if (!isErr && call.tool !== "list_commands" && A.toolList.length) {
            ...
            if (A.toolCallsSinceReminder >= REMIND_TOOLS_EVERY) {
              ...
              // Scope the reminder to the primary Unity server, exactly like
              ...
              // needs the primary Unity set; addon commands were listed on demand
              // and the bridge routes by name regardless.
              const unity = A.toolList.filter((t) => (t.server || "unity") === "unity");
              toSend += CS.toolsReminder(unity) + "\n" + CS.memoryNudge();
  ...
  function stopLoop() {
    ...
    if (A.toolRunning && A.toolItem) {
      ...
      decorate.toolBox(A.toolItem, A.toolName, "err", "stopped", true, "", CS.toolCategory(A.toolName));
    }
  ...
  async function startSession() {
    ...
    // restored execute_code can't leak into the freshly started conversation.
    ...
    try {
      ...
      if (!A.toolList.length) {
        ui.banner("warn", "Bridge or Unity offline",
          "Could not fetch Unity tools. Run start.bat and make sure the Unity Editor is open, then try again.");
        return;
      }
      ...
      const prompt = CS.buildSystemPrompt({ siteName: P.displayName, customPrompt: ui.getCustomPrompt() });
      ...
      if (startRes.kind === "tool" && startRes.calls && startRes.calls.length === 1 &&
        ...
        // Unity down short-circuits list_commands into a plain "offline" note
        ...
        // (the bridge keeps advertising Unity's catalogue even with no Editor
        // attached, so A.toolList still has entries that were never actually
        // usable this boot).
        if (/Unity Editor is currently OFFLINE/.test(toolFeedback)) {
          decorate.toolBox(startRes.item, "Loading commands", "err", "Unity offline", true);
        } else {
          // Count what the model ACTUALLY received: list_commands is scoped to the
          // primary Unity server (main.js ~629), so showing A.toolList.length (every
          // connected server merged - Unity + Blender + addons) overstated the boot
          // count and made it look like all servers were loaded at once. Count the
          // Unity-scoped tools instead, matching the real result.
          const unityCount = A.toolList.filter((t) => (t.server || "unity") === "unity").length;
          decorate.toolBox(startRes.item, "Loading commands", "done", `${unityCount} commands`, true);
        }
      ...
      ui.setStarted(true);
      ui.toast(`Agent ready. Ask ${P.displayName} to build something in Unity.`);
    } catch (e) {
  ...
  const ICONS = {
    ...
    unity:   SVG('<path d="M13 2 3 7v10l10 5 10-5V7z"/><path d="M3 7l10 5 10-5M13 22V12"/>'),
  ...
  const SPIN = '<span class="cs-spin"></span>';
  ...
  function resetDecoration(item) {
    const chip = item.querySelector(".cs-chip");
    if (chip) chip.remove();
    item.classList.remove("cs-hidden");
    item.querySelectorAll(".cs-tool-hide").forEach((e) => e.classList.remove("cs-tool-hide"));
    item.querySelectorAll(".cs-cmd-mask").forEach((e) => e.classList.remove("cs-cmd-mask"));
    delete item.dataset.cs;
    delete item.dataset.csig;
    delete item.dataset.cphase;
    delete item.dataset.zStopped;
    delete item.dataset.zRegenLen;
    delete item.dataset.zRegenAt;
    delete item.__csChip;
  }
  ...
  const decorate = {
    ...
    chip(item, opts) {
      ...
      let chip = item.querySelector(".cs-chip");
      const hasBody = !!body;
      ...
      // <span class="cs-spin"> and restarts its CSS animation each time, so the
      ...
      if (chip.dataset.csig !== sig) {
        ...
        chip.className = `cs-chip cat-${category} ${cls || ""}`;
        chip.innerHTML =
          `<div class="cs-chip-head">` +
            `<span class="cs-chip-ic">${iconFor(category, phase)}</span>` +
            `<span class="cs-chip-tx"></span>` +
            `<span class="cs-chip-dt"></span>` +
            (hasBody ? `<span class="cs-chip-cv">${SVG('<polyline points="6 9 12 15 18 9"/>')}</span>` : "") +
          `</div>` +
          (hasBody ? `<div class="cs-chip-body"><pre></pre></div>` : "");
        chip.querySelector(".cs-chip-tx").textContent = label;
        if (detail) chip.querySelector(".cs-chip-dt").textContent = detail;
        if (hasBody) {
          chip.querySelector(".cs-chip-body pre").textContent = body;
          const head = chip.querySelector(".cs-chip-head");
      ...
      if (whole) {
        ...
        item.classList.remove("cs-hidden");
        // findToolBlockSpot ALSO applies the .cs-tool-hide classes (its real job);
    ...
    // and/or the .cs-tool-hide classes stripped). The loop owns the label/phase,
    // so we rebuild from the stored opts rather than re-running classification.
    ensureOwnedChip(item) {
      const opts = item.__csChip;
      if (!opts) return;
      const chipEl = item.querySelector(".cs-chip");
      const chipGone = !chipEl;
      let rawVisible = false;
      if (!opts.whole) {
        // NOTE the thinking exclusion: reasoning models QUOTE the command
        // JSON/###CSHARP### in their think area, which the camouflage never hides
        ...
        rawVisible = [...item.querySelectorAll("pre, p, [class*='code'], .cm-line")].some(
          (e) => !e.closest(".cs-tool-hide") && !e.closest(".cs-chip") &&
                 ...
                 // the .cs-tool-hide class) live on a child wrapper instead.
                 ...
                 !e.querySelector(".cs-tool-hide") &&
                 CSParse.hasCommandShape(e.textContent || ""));
      }
    ...
    toolBox(item, name, phase, detail, owned, body, category) {
      ...
      if (item.dataset.cphase !== phase) {
        diag("chip.phase", {
          name, from: item.dataset.cphase || "(new)", to: phase,
      ...
      this.chip(item, {
        ...
        category: category || CS.toolCategory(name), phase, cls,
      });
      item.dataset.cphase = phase;
      if (owned) item.dataset.zloop = "1";
    },

    classify(item, next) {
      if (item.dataset.zloop) { this.ensureOwnedChip(item); return; } // loop owns it
      const txt = P.classifyText(item, ".cs-chip"); // excludes thinking AND our chip
      ...
      // .cs-tool-hide classes are wiped while the dataset flags on the turn
      ...
      const chipGone = !item.querySelector(".cs-chip");

      // 1. System-prompt bootstrap turn → animated while starting, gear when done.
      if (txt.includes(CS.SYS_MARKER)) {
        const phase = A.starting ? "run" : "sys";
        if (item.dataset.cs !== "sys" || item.dataset.cphase !== phase || chipGone) {
      ...
      if (P.isUserItem(item) && CSParse.isInjectedFeedback(txt)) {
        ...
        if (item.dataset.csig !== sig || !item.classList.contains("cs-hidden") || chipGone) {
          this.chip(item, {
            label: m ? `${m[1]} · result` : "result",
            category: hasImg ? "screen" : m ? CS.toolCategory(m[1]) : "tool",
            body: txt, phase: isErr ? "err" : "result",
            cls: isErr ? "err" : "result", whole: true,
          });
          item.dataset.csig = sig;
        }
        return;
      }
      ...
      // code block on a COLD page load - the dataset.csCode cache only helps
      ...
      if (P.isAssistantItem(item) && !CSParse.hasCommandShape(txt) &&
          next && P.isUserItem(next)) {
        const nt = P.classifyText(next, ".cs-chip");
        const m = nt.match(/^\s*Output of '([^']+)'/);
        if (m) {
          const isErr = /^\s*ERROR\b/.test(nt);
          const phase = isErr ? "err" : "done";
          if (item.dataset.cphase !== phase || chipGone) {
      ...
      // ONLY in a real CipherScript session (started or bootstrapping). Without
      ...
      if (P.isAssistantItem(item) && CSParse.hasCommandShape(txt) &&
        ...
        if (item.dataset.zRegenLen) {
          ...
          if (!replaced && !expired) {
            const nm = CSParse.toolNameFromText(txt) || "command";
        ...
        if (stopped && item.dataset.zStopped !== "1") {
          item.dataset.zStopped = "1";
          diag("chip.rehalt", { name: CSParse.toolNameFromText(txt) });
        }
        ...
        // wiping the chip, the zloop ownership AND the __csChip opts. Without
        ...
        if (!stopped && A.running && !A.toolRunning && A.toolSettle &&
            ...
            CSParse.toolNameFromText(txt) === A.toolName) {
        ...
        const resultAfter = next && P.isUserItem(next) &&
          CSParse.isInjectedFeedback(P.classifyText(next, ".cs-chip"));
        ...
        // reload-restored generation (the "execute_code leaked into the new chat"
        ...
        if (phase === "done" && next && P.isUserItem(next)) {
          ...
          if (CSParse.isInjectedFeedback(nt) && feedbackIsError(nt)) {
            phase = "err"; detail = "error";
            if (item.dataset.cphase !== "err") diag("chip.errSettle", { name: CSParse.toolNameFromText(txt) });
          }
        }
        ...
        const rawVisible = [...item.querySelectorAll("pre, p, [class*='code'], .cm-line")].some(
          (e) => !e.classList.contains("cs-tool-hide") && !e.closest(".cs-tool-hide") &&
                 !e.closest(".cs-chip") && !(P.thinkingSel && e.closest(P.thinkingSel)) &&
        ...
        // name alone wouldn't reveal it (parity with the vision tool). The
        ...
        const nm = CSParse.toolNameFromText(txt);
        const cat = A.imageTools.has(bareToolName(nm)) ? "screen" : undefined;
        const chipNow = item.querySelector(".cs-chip");
        ...
        if (item.dataset.cphase !== phase || chipGone || rawVisible || catStale || drifted) {
      ...
      const haltedTurn =
        item.dataset.zStopped === "1" ||
        (A.userStopped && item === P.lastAssistant());
      if (haltedTurn && P.isAssistantItem(item) && item.dataset.cphase !== "err"
          && item.querySelector(".cs-chip")) {
        const tx = item.querySelector(".cs-chip-tx");
        const name = CSParse.toolNameFromText(txt) || (tx && tx.textContent) || "tool";
      ...
      if (!txt.trim() && (item.dataset.cphase || item.dataset.cs)) return;
      ...
      if (item.dataset.cs || item.dataset.cphase || item.querySelector(".cs-chip")) {
        ...
        diag("chip.reset", { was: item.dataset.cphase || item.dataset.cs || "chip-only" });
    ...
    sweep() {
      ...
      for (const chip of document.querySelectorAll(".cs-chip.run")) {
        let item = chip.parentElement;
        while (item && !(item.dataset && item.dataset.zStopped)) item = item.parentElement;
        if (item && item.dataset.cphase !== "err") {
          const tx = chip.querySelector(".cs-chip-tx");
  ...
  const ui = (() => {
    ...
    let bridgeOk = false, unityDown = false, placeDown = false, appDown = false, addonOk = false, unityProcUp = false;
    ...
    function build() {
      ...
      // menu (other AI sites, custom prompt). No floating panel,
      // no overlay on the input - the composer stays fully usable for plain chat.
      root.innerHTML = `
        <div id="cs-bar">
          <span id="cs-dot" class="off" title=""></span>
          <span id="cs-brand"><img class="cs-logo" src="${chrome.runtime.getURL("logo.png")}" alt="">CipherScript <span class="cs-free">v${EXT_VERSION}</span></span>
          <span id="cs-state"></span>
          <button id="cs-action"></button>
          <button id="cs-stop" hidden>■ Stop</button>
          <button id="cs-switch" aria-label="Switch AI and options" title="Switch AI, custom prompt"><span id="cs-switch-name"></span><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg></button>
        </div>
        <div id="cs-menu" hidden></div>
        ${P.unstableWarning ? `<button id="cs-unstable" aria-label="Provider may be unstable" hidden>⚠ unstable</button>` : ""}
      `;
      document.documentElement.appendChild(root);
      bar = root.querySelector("#cs-bar");
      dot = root.querySelector("#cs-dot");
      brandEl = root.querySelector("#cs-brand");
      stateEl = root.querySelector("#cs-state");
      actionBtn = root.querySelector("#cs-action");
      stopBtn = root.querySelector("#cs-stop");
      switchBtn = root.querySelector("#cs-switch");
      const swName = root.querySelector("#cs-switch-name");
      if (swName) swName.textContent = P.displayName || P.id;
      menuEl = root.querySelector("#cs-menu");
      bar.classList.add(`cs-prov-${P.id}`); // lets CSS tune per-site (e.g. font)
      ...
      document.documentElement.classList.add(`cs-site-${P.id}`);

      actionBtn.addEventListener("click", onActionClick);
      stopBtn.addEventListener("click", stopLoop);
      unstableEl = root.querySelector("#cs-unstable");
      if (unstableEl) {
      ...
      buildMenu();
      // The model button opens the panel at the top with Switch AI.
      const toggleMenu = () => {
    ...
    let customPrompt = "";
    try {
      chrome.storage.local.get("csCustomPrompt", (r) => {
        if (r && typeof r.csCustomPrompt === "string") {
          customPrompt = r.csCustomPrompt;
          syncMenuPrompt();
        }
      });
    } catch {}
    ...
    function syncMenuPrompt() {
      const ta = root && root.querySelector("#cs-set-text");
    ...
    // ADDONS: the Unity server stays primary and is never in this list. Each
    ...
    try {
      chrome.storage.local.get("csCustomMcpServers", (r) => {
        if (r && Array.isArray(r.csCustomMcpServers)) {
          customMcpServers = r.csCustomMcpServers;
    ...
    function saveCustomMcpServers() {
      try { chrome.storage.local.set({ csCustomMcpServers: customMcpServers }); } catch {}
    }
    ...
    function mergedMcpServers() {
      const live = ((A.bridge && A.bridge.servers) || []).filter((sv) => sv.id !== "unity");
    ...
    // Derive a config-safe server id from a display name (unity is reserved).
    function mcpSlug(name) {
      let s = String(name || "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
      if (!s || s === "unity") s = `addon-${s || "server"}`;
      let id = s, n = 2;
    ...
    // ── The "more" menu (⋯) ─────────────────────────────────────────────────
    // One popover holding every secondary control: other AI sites and the custom
    // prompt. Opens above the bar.
    function buildMenu() {
      ...
      for (const s of AI_SITES) {
        ...
        const label = `<span class="cs-site-name"><span>${s.name}</span><span class="cs-site-host">${hostOf(s.url)}</span></span>`;
        sites += current
          ? `<div class="cs-site-opt cs-site-here">${label}<span class="cs-site-badge">active</span></div>`
          : `<button class="cs-site-opt" data-u="${s.url}">${label}<span class="cs-site-go">&rarr;</span></button>`;
      }
      const esc = (s) => String(s).replace(/[&<>"']/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" }[c]));
      const mergedServers = mergedMcpServers();
      // Unity always heads the list - greyed out, no health dot (its own status
      // is already the main CipherScript dot elsewhere) and no remove button (it's
      // the primary server, protected bridge-side too).
      let mcpList =
        `<div class="cs-mcp-item cs-mcp-item-primary"><div class="cs-mcp-info"><span class="cs-mcp-name">Unity Editor</span><span class="cs-mcp-url">primary - always connected</span></div></div>`;
      mergedServers.forEach((s, i) => {
        ...
        mcpList += `<div class="cs-mcp-item"><span class="cs-mcp-health cs-mcp-health-${healthClass}" title="${healthTitle}"></span><div class="cs-mcp-info"><span class="cs-mcp-name">${esc(s.name)}</span><span class="cs-mcp-url">${esc(s.command || s.id)}</span></div><button class="cs-mcp-remove" data-id="${esc(s.id)}" title="Remove">✕</button></div>`;
      });
      menuEl.innerHTML =
        `<div class="cs-menu-head"><span class="cs-menu-logo"><img class="cs-logo" src="${chrome.runtime.getURL("logo.png")}" alt="">CipherScript</span><span class="cs-menu-tag">v${EXT_VERSION}</span></div>
         <section class="cs-menu-sec">
           <div class="cs-sec-label"><span>Switch AI</span></div>
           ${sites}
         </section>
         <section class="cs-menu-sec">
           <div class="cs-sec-label"><span>Custom prompt</span></div>
           <div class="cs-menu-note">Added below the system prompt on every new session. The built-in prompt can't be edited.</div>
           <textarea id="cs-set-text" rows="4" placeholder="e.g. Always comment your C# code. Use Unity's Input System."></textarea>
           <div class="cs-set-row"><button id="cs-set-save">Save</button><span id="cs-set-status"></span></div>
         </section>
         <section class="cs-menu-sec">
           <div class="cs-sec-label"><span>MCP servers</span></div>
           <div class="cs-menu-note">Unity Editor is always connected (primary). Add another MCP server (e.g. Blender, Sketchfab) as an addon - the bridge restarts briefly to load it. Experimental.</div>
           ${mcpList}
           <div class="cs-mcp-sep"></div>
           <input id="cs-mcp-name" class="cs-mcp-field" placeholder="Name, e.g. Blender" />
           <input id="cs-mcp-url" class="cs-mcp-field" placeholder="Start command, e.g. npx -y @some/mcp-server" />
           <div class="cs-set-row"><button id="cs-mcp-add">Add server</button><span id="cs-mcp-status"></span></div>
         </section>`;
      const open = (url) => { try { window.open(url, "_blank", "noopener"); } catch {} menuEl.hidden = true; };
      menuEl.querySelectorAll("button.cs-site-opt").forEach((b) =>
        b.addEventListener("click", () => open(b.dataset.u)));
      const ta = menuEl.querySelector("#cs-set-text");
      const saveBtn = menuEl.querySelector("#cs-set-save");
      const status = menuEl.querySelector("#cs-set-status");
      ta.value = customPrompt;
      saveBtn.addEventListener("click", () => {
        customPrompt = ta.value;
        try { chrome.storage.local.set({ csCustomPrompt: customPrompt }); } catch {}
        status.textContent = "Saved ✓";
        setTimeout(() => { status.textContent = ""; }, 1600);
      });
      const mcpNameEl = menuEl.querySelector("#cs-mcp-name");
      const mcpUrlEl = menuEl.querySelector("#cs-mcp-url");
      const mcpStatus = menuEl.querySelector("#cs-mcp-status");
      const mcpAddBtn = menuEl.querySelector("#cs-mcp-add");
      ...
      function setMcpBusy(on, label) {
        ...
        menuEl.querySelectorAll(".cs-mcp-remove").forEach((b) => (b.disabled = on));
        mcpStatus.innerHTML = on
          ? `<span class="cs-mcp-spin-row"><span class="cs-mcp-spin"></span>${label || "Restarting bridge…"}</span>`
          : "";
      }

      menuEl.querySelectorAll(".cs-mcp-remove").forEach((b) =>
    ...
    //  • starting        → spinner, "Starting the Unity agent…"
    ...
    function renderBar() {
      ...
      if (A.starting) {
        toneClass = "starting";
        indicator = `<span class="cs-spin"></span>`;
        msg = `Starting the Unity agent…`;
        ...
        // server's per-server health count (Unity + addons like Blender) - NOT the
        // Unity-only count, which made the total drop after a reload.
        ...
        // "N tools" only means the MCP server itself is up - it advertises its
        // full catalogue even with no Unity Editor attached (see probe_unity()
        // in bridge.py), so showing it while the Editor isn't actually usable
        ...
        if (A.bridge && A.bridge.connected === false) {
          // placeDown/appDown/unityDown are all false in this case (they're
        ...
        } else if ((placeDown || appDown || unityDown) && addonOk) {
          // DEGRADED session by CHOICE: the user started the agent with Unity
          // down but other MCP server(s) alive (the "Start agent (Unity
          ...
          // red imperative "open Unity" nag on screen for the whole
          // session (warn=false → no cs-state-warn red text). The full nag
          // still shows when NO server is usable (the branches below).
          toneClass = "warn";
          msg = `<b>Agent active</b>${tools ? ` · ${tools} tools` : ""} · Unity offline`;
        } else if (placeDown) {
          toneClass = "warn"; warn = true;
          msg = `<b>Agent active</b> · open a Unity project in the Editor`;
        } else if (appDown || unityDown) {
          toneClass = "warn"; warn = true;
          msg = unityProcUp
            ? `<b>Agent active</b> · Unity is open but not connected - open <b>Window &gt; Unity MCP Toolkit</b> in the Editor`
            : `<b>Agent active</b> · open the Unity Editor & start its MCP server`;
        } else {
          toneClass = "active";
        ...
        if (bridgeOk) {
          ...
          label = "▶ Start Unity agent"; kind = "start";
        } else if (addonOk) {
          // Unity is down but another MCP server is live: allow a DEGRADED start
          // (yellow). The agent runs on the other server(s); Unity tools stay
          // unavailable until the Editor is back. Button enabled, but visibly warned.
          toneClass = "warn"; warn = true;
          msg = !A.bridge.connected
            ? `Run <b>start.bat</b> on your PC.`
            : unityProcUp
              ? `<b>Unity open but not connected</b> - open <b>Window &gt; Unity MCP Toolkit</b> in the Editor, or start without it.`
              : `<b>Unity Editor offline</b> - start with your other MCP server(s).`;
          label = "▶ Start agent (Unity offline)"; kind = "start-degraded";
        } else {
          toneClass = "warn"; warn = true;
          msg = !A.bridge.connected
            ? `Run <b>start.bat</b> on your PC.`
            : placeDown
              ? `Open a <b>Unity project</b> in the Editor.`
              : (appDown || unityDown) && unityProcUp
                ? `Unity is open but not connected - open <b>Window &gt; Unity MCP Toolkit</b> in the Editor.`
                : appDown
                  ? `Open <b>Unity Editor</b> &amp; start its MCP server (Window &gt; Unity MCP Toolkit).`
                  : unityDown
                    ? `Open <b>Unity Editor</b> &amp; start its MCP server (Window &gt; Unity MCP Toolkit).`
                    : `Open <b>Unity Editor</b> for the tools.`;
          label = "▶ Start Unity agent"; kind = "start";
        }
      ...
      // The AI selector appears once the agent is actually running - so the
      // pre-start bar isn't cluttered with options that only matter mid-session.
      // (A.started is ambiguous with `warn` tone - which occurs both started-with-
      // bridge-down and standby-with-bridge-down - so it's tracked explicitly in
      // the signature.)
      ...
      lastBarSig = sig;
      // Set the tone WITHOUT clobbering other classes (e.g. cs-bar-inline, which
      ...
      stateEl.innerHTML = indicator + `<span class="cs-state-txt">${msg}</span>`;
      stateEl.classList.toggle("cs-state-warn", warn);
    ...
    function setStatus(s) {
      ...
      // CipherScript status tracks ONLY the primary Unity MCP server. Every other
      // server is an addon and must NEVER make the dot/gate look connected while
      // Unity itself is down. Old bridges don't send per-server health, so fall
      // back to the aggregate signals they do send (mcpAlive / total tools).
      const un = servers.find((x) => x.id === "unity");
      const mcpUp = un ? !!un.alive : (!!s.mcpAlive || servers.some((x) => x.alive));
      // Unity-only count drives the connectivity gate (the dot must never look
      // green off an addon while Unity itself is down)...
      const unityTools = un ? (un.tools || 0) : (s.tools || 0);
      const mcpOk = s.connected && (mcpUp || unityTools > 0);
      // ...but the DISPLAYED count is the aggregate across every server (Unity +
      // addons like Blender), so it stays consistent with the bar and doesn't
      // under-report when addon servers are loaded.
      const totalTools = servers.reduce((n, x) => n + (x.tools || 0), 0) || s.tools || unityTools;
      // unity === false means the MCP server answered but the Unity Editor is not
      // USABLE (closed, or its MCP server not running). unityApp tells the two
      // sub-cases apart:
      //   unityApp === false → no Editor connected at all (closed OR its MCP
      //                        server is not running - indistinguishable).
      //   unityApp === true  → Editor connected. (Unity always has a project
      //                        open when connected, so the old "app open but no
      //                        place loaded" legacy sub-case has no Unity
      //                        equivalent - place tracks app.)
      // null/undefined = unknown (old bridge / probe busy) → don't degrade.
      const unityOff = mcpOk && s.unity === false;
      const noApp = unityOff && s.unityApp === false;
      const noPlace = unityOff && s.unityApp === true;
      const ok = mcpOk && !unityOff;
      dot.className = s.connected ? (ok ? "on" : "warn") : "off";
      // Unity PROCESS running on the machine (bridge-side tasklist check).
      // Splits noApp into its two truly different situations: Editor not
      // launched at all vs Editor OPEN but its MCP server never started/connected
      // to the bridge. The Unity MCP Toolkit C# plugin only connects when the server
      // is started (Window > Unity MCP Toolkit > Start server), so for the second
      // case "open Unity" is dead-end advice - the action that works is opening
      // Window > Unity MCP Toolkit inside the already-open Editor.
      const procUp = s.unityProc === true;
      let txt;
      if (!s.connected) txt = "Bridge offline, run start.bat";
      else if (!mcpOk) txt = "Bridge OK, open the Unity Editor";
      else if (noPlace) txt = "Unity Editor is open but not connected - start its MCP server";
      else if (noApp) txt = procUp
        ? "Unity is open but not connected - in the Editor, open Window > Unity MCP Toolkit (or toggle its server off/on)"
        : "Unity Editor not connected - open it and start the MCP server (Window > Unity MCP Toolkit)";
      else if (unityOff) txt = "Editor not connected, start the MCP server in Unity (Window > Unity MCP Toolkit)";
      ...
      appDown = noApp;
      unityProcUp = procUp;
      // A non-Unity MCP (Blender, Sketchfab, ...) that is actually alive. When
      // Unity itself is down but such a server is present, the session can still
      // start in a DEGRADED mode - the agent just can't touch Unity until the
      // Editor is back. Gated on s.connected so a dropped bridge never reads as
      // usable.
      addonOk = !!s.connected && servers.some((x) => x.id !== "unity" && x.alive && (x.tools || 0) > 0);
    ...
    function bridgeAlert(on) {
      ...
      b.className = "cs-banner limit";
      b.innerHTML = `<div class="cs-banner-t">⚠ Lost connection to CipherScript</div>
        <div class="cs-banner-m">The CipherScript bridge stopped on your PC. Restart it (run start.bat and keep the Unity Editor open): the agent will reconnect automatically as soon as it is detected again.</div>
        <div class="cs-banner-acts"><button class="cs-banner-x">Close</button></div>`;
      b.querySelector(".cs-banner-x").addEventListener("click", () => { b.remove(); if (bridgeBannerEl === b) bridgeBannerEl = null; });
      root.appendChild(b);
      bridgeBannerEl = b;
    }
    ...
    // the Start button so they discover how to enable Unity control.
    let nudged = false;
    function nudgeStart() {
      if (A.started || !P.isFreshChat()) return;
      if (!nudged) {
        nudged = true;
        toast("Tip: click “▶ Start Unity agent” to let the AI control the Unity Editor.");
      }
      if (!actionBtn) return;
      actionBtn.classList.add("cs-flash");
      setTimeout(() => actionBtn.classList.remove("cs-flash"), 1200);
    }
    ...
    // `.cs-light` to <html>; overlay.css then flips to readable light colours.
    ...
    function placeBar() {
      ...
      // #cs-root from <html>, taking the bar with it - and nothing re-adds it, so
      ...
      if (mount) {
        ...
        if (!bar.classList.contains("cs-bar-inline")) {
          bar.classList.add("cs-bar-inline");
        ...
        bar.classList.toggle("cs-bar-inside", !!mount.inside);
      ...
      // node into (e.g. Kimi's Vue tree - inserting #cs-bar there makes Vue's
      // next diff reuse the bar node as a host and nest the editor inside it).
      // So we keep the bar in our own #cs-root, position it (position:fixed) to
      ...
      if (anchorEl && anchorEl.isConnected) {
        bar.classList.remove("cs-bar-inline", "cs-bar-inside");
        bar.classList.add("cs-bar-anchored");
        ...
        if (menuEl && !menuEl.hidden) {
          bar.classList.remove("cs-bar-inline"); // ensure fixed geometry for menu math
    ...
    function inputCover(on) {
      ...
      if (!on) {
        ...
        if (ed) ed.classList.remove("cs-typing");
        cancelAnimationFrame(coverRaf);
        return;
      }
      if (!ed) return;
      ed.classList.add("cs-typing"); // make the typed text itself invisible
      if (!cover) {
        cover = document.createElement("div");
        cover.id = "cs-input-cover";
      ...
      const place = () => {
        ...
        if (!e.classList.contains("cs-typing")) e.classList.add("cs-typing");
        ...
        // .cs-typing height cap, so the editor balloons to ~1500px while its
        ...
        // are unchanged.
        // Hard ceiling: even though .cs-typing caps the editor's visual height
    ...
    function banner(kind, title, msg) {
      const b = document.createElement("div");
      b.className = `cs-banner ${kind}`;
      b.innerHTML = `<div class="cs-banner-t"></div><div class="cs-banner-m"></div>
        <div class="cs-banner-acts">
          <button class="cs-banner-x">Close</button>
        </div>`;
      b.querySelector(".cs-banner-t").textContent = title;
      b.querySelector(".cs-banner-m").textContent = msg;
      b.querySelector(".cs-banner-x").addEventListener("click", () => b.remove());
      root.appendChild(b);
    }

    // Left-hand CipherScript popup showing the latest screen_capture. Fed from the
    ...
    function showImages(images, toolName) {
      root.querySelectorAll(".cs-shot").forEach((e) => e.remove());
      const wrap = document.createElement("div");
      wrap.className = "cs-shot";
      const hdr = document.createElement("div");
      hdr.className = "cs-shot-hdr";
      const ttl = document.createElement("span");
      ttl.className = "cs-shot-ttl";
      ttl.textContent = `${toolName} · ${images.length} image${images.length > 1 ? "s" : ""}`;
      const close = document.createElement("button");
      close.className = "cs-shot-x";
      close.textContent = "✕";
      ...
      body.className = "cs-shot-body";
      for (const img of images) {
        const el = document.createElement("img");
        el.className = "cs-shot-img";
    ...
    build();
    return { setStatus, setStarted, setStarting, showStop, markStopping, inputCover, toast, banner, showImages, nudgeStart, updateStartGate, getCustomPrompt, getCustomMcpServers, openMenu: () => openMenuFn && openMenuFn() };
  })();
  ...
  function setChipDetail(item, text) {
    const dt = item && item.querySelector(".cs-chip .cs-chip-dt");
  ...
  function setChipLabel(item, text) {
    const tx = item && item.querySelector(".cs-chip .cs-chip-tx");
  ...
  // CipherScript's own UI). A genuine "regenerate ↻" is always such a click;
  ...
  let _userClickAt = 0;
  document.addEventListener("click", (e) => {
    if (e.isTrusted && !(e.target && e.target.closest && e.target.closest("#cs-root"))) {
  ...
  setInterval(() => {
    ...
    if (P.hasRegenerate !== false &&
      ...
      // the extension's own "■ Stop" is inside #cs-root and never updates
      ...
      if (clickAge < 2500 && clickAfterStop > 400) {
        ...
        if (it) {
          ...
          // turn node, and without this the previous execute_code chip (with its
          ...
          try {
            it.dataset.zRegenLen = String(P.classifyText(it, ".cs-chip").length);
    ...
    // The bare isHardGenerating() term is gated on a live CipherScript session: on
    ...
    if (A.stopping && !A.running && !A.toolRunning) {
      if (A.started && P.isHardGenerating()) {
        ...
        // 2026-07: two stray stop.retry clicks after a CS Stop made the next
    ...
    if (A.toolRunning && A.toolItem) {
      const s = elapsedOn(A.toolItem, "csToolT0", A.toolStart).toFixed(1);
  ...
  chrome.runtime.onMessage.addListener((msg) => {
    if (msg && msg.type === "cs-status") {
      ui.setStatus({ connected: msg.connected, mcpAlive: msg.mcpAlive, unity: msg.unity, unityApp: msg.unityApp, unityProc: msg.unityProc, tools: msg.tools, servers: msg.servers });
    }
    if (msg && msg.type === "cs-open-menu") {
      ui.openMenu(); // from the popup's Settings button — opens at the top (Switch AI / custom prompt)
    }
  });
  ...
  function rememberSession(path) {
    ...
    try { chrome.storage.local.set({ csStartedSessions: [...startedSessions].slice(-300) }); } catch {}
  }
  // Load the persisted set once, then re-sync.
  try {
    chrome.storage.local.get("csStartedSessions", (r) => {
      if (r && Array.isArray(r.csStartedSessions)) {
        for (const p of r.csStartedSessions) startedSessions.add(p);
        syncSessionState();
      }
    });
  } catch {}
  // A conversation IS a CipherScript session if any rendered turn carries a
  // telltale artefact: the system-prompt marker, an injected tool-result /
  // system-note turn, or a CipherScript command an assistant wrote. Works even
  ...
  function domHasZsSignal() {
    for (const it of P.allItems()) {
      ...
      if (txt.includes(CS.SYS_MARKER)) return true;
      ...
      // merely CONTAINS {"command":...} / ###CSHARP### is NOT proof of a session:
      ...
      // key set (startedSessions / csStartedSessions in rememberSession), not
      // by this heuristic.
    }
    return false;
  }
  function syncSessionState() {
    ...
    // via the SITE's own new-chat (not CipherScript's button), the loop is bound to
    ...
    // the fresh chat instead of "Start Unity agent". The "/app" → "/app/<id>" id
  ...
  function preHideWholeItems() {
    ...
    if (A.injectHideUntil && Date.now() < A.injectHideUntil) {
      ...
      if (last && !last.classList.contains("cs-hidden") &&
          users.length > (A.injectPreUser || 0)) {
        last.classList.add("cs-hidden");
        A.injectHideUntil = 0; // one-shot: this turn is now masked
        diag("result.prehide", { users: users.length });
      }
    }
    for (const item of items) {
      if (item.classList.contains("cs-hidden")) continue;
      const txt = P.classifyText(item, ".cs-chip");
      if (txt.includes(CS.SYS_MARKER) ||
          (P.isUserItem(item) && CSParse.isInjectedFeedback(txt))) {
        item.classList.add("cs-hidden");
      }
    }
  }
  ...
  setInterval(() => {
    ...
    // A.bootBaselineId). Guards the "execute_code leaked into the new chat" bug.
    ...
    if (after && P.isUserItem(after) &&
        CSParse.isInjectedFeedback(P.classifyText(after, ".cs-chip"))) return;
    const txt = P.itemText(item);
    if (!CSParse.hasToolSignature(txt)) return;
    ...
    if (!CSParse.parseToolCalls(txt).length) return;
  ...
  log(`CipherScript content script ready (provider: ${P.id})`);
})();
