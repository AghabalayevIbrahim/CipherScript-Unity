...
// core (core/main.js) only ever talks to the CSProvider interface this file
...
const CSProvider = (() => {
  ...
  // Scope to the SITE's composer only: never match CipherScript's own injected
  // UI (e.g. the settings textarea #cs-set-text in #cs-root). Otherwise on the
  ...
  const getEditor = () => {
    ...
    // returned it - and barMount() then dragged the whole CipherScript bar INTO the
  ...
  function barMount() {
    ...
    if (before && before.id === "cs-bar") before = before.nextElementSibling;
  ...
  function enforceComposer(reason) {
    ...
    // off) without CipherScript reverting their choice every frame.
  ...
  function streamText(item) {
    ...
    const replyTxt = [...item.querySelectorAll(S.markdown)]
      .filter((m) => !m.closest(S.thinking) && !m.closest(".cs-chip"))
      .map((m) => m.textContent)
      .join("");
    return thinkTxt + "\n" + replyTxt;
  }
  ...
  function truncateForSend(text) {
    ...
    const marker =
      `\n\n[…CipherScript: result truncated to fit DeepSeek's ${SEND_CAP}-character ` +
    ...
    try {
      return [...document.querySelectorAll("img")].filter(
        (im) => !im.closest(S.chatItem) &&
          // blob: = pending local preview; the alt (our "cipherscript_..." filename)
          ...
          (/^blob:/.test(im.getAttribute("src") || "") || /^cipherscript_/.test(im.getAttribute("alt") || "")));
    } catch { return []; }
  };
  ...
  //   isStarted():bool        - a CipherScript session exists in this chat
  ...
  function findToolBlockSpot(item, chip) {
    const P = CSParse;
    const hasStart = (t) => P.CS_START_RE.test(t) || t.includes("###mcp_tool###");
    const hasEnd = (t) => P.CS_END_RE.test(t) || t.includes("###end_mcp_tool###") || t.includes("###end-mcp_tool###");
    ...
    for (const container of containers) {
      ...
      while (i < kids.length) {
        ...
        for (let k = runStart; k <= runEnd; k++) {
          ...
          hide.classList.add("cs-tool-hide");
  ...
  return {
    ...
    init({ diag: d } = {}) {
      ...
      // confirmed from the page (read document.documentElement.dataset.csDsVer).
      // BUMP DS_VER on meaningful deepseek.js changes worth verifying live.
      try { document.documentElement.setAttribute("data-cs-ds-ver", "2026-07_vision-badge-priority"); } catch {}
    },
    // turns
