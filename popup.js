...
function render(s) {
  ...
  const unityOff = mcpOk && s.unity === false; // MCP up but no Unity Editor attached
  const ok = mcpOk && !unityOff;
  dot.className = "dot " + (s.connected ? (ok ? "on" : "warn") : "");
  state.textContent = s.connected
    ? (ok ? "Connected · Unity Editor ready"
        : unityOff ? "Editor not connected · start the MCP server in Unity"
        : "Bridge OK · open the Unity Editor")
    : "Bridge offline";
...
document.getElementById("restart").addEventListener("click", (e) => {
  ...
  chrome.runtime.sendMessage({ type: "restart_mcp" }, () => {
    e.target.textContent = "⟳ Restart Unity MCP server";
...
document.getElementById("settings").addEventListener("click", () => {
  ...
  chrome.tabs.query({}, (tabs) => {
    ...
    if (anySupported) {
      chrome.tabs.sendMessage(anySupported.id, { type: "cs-open-menu" });
...
chrome.runtime.onMessage.addListener((msg) => {
  if (msg && msg.type === "cs-status") render(msg);
});
refresh();
