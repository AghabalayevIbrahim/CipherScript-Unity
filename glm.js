...
const CSProvider = (() => {
  ...
  const S = {
    ...
    // so "cipherscript_….jpg JPG 179 KB Output of 'screen_capture'…" failed to match
  ...
  function textWithout(root, excludeSel) {
    if (!root) return "";
    const skip = S.thinking + ", .cs-chip, " + S.attachment + (excludeSel ? ", " + excludeSel : "");
    let t = "";
    const walk = (n) => {
  ...
  // reconciles the composer subtree, so we must NOT insert our #cs-bar into it
  ...
  // bar in #cs-root, positions it to hug that card's top edge at full width, and
  ...
  // identity) with .cs-cmd-mask so a CSS rule re-hides recreated wrappers with
  ...
  function findToolBlockSpot(item /*, chip */) {
    ...
    for (const cc of item.querySelectorAll(S.copyCodeBtn)) {
      ...
      if (!wrap || wrap.closest(".cs-chip")) continue;
      if (CMD_SHAPE.test(wrap.textContent || "")) {
        wrap.classList.add("cs-tool-hide");
    ...
    [...reply.children].forEach((el) => {
      if (el.classList.contains("cs-chip") || el.closest(S.thinking) ||
