...
const CSProvider = (() => {
  ...
  // CipherScript bar stuck on "Agent active" with no way to start a new session.
  ...
  function setInputLock(on) {
    ...
    if (on) ph.setAttribute("data-cs-locked", "1");
    else ph.removeAttribute("data-cs-locked");
  }
  ...
  function truncateForSend(text) {
    ...
    const marker = (what) =>
      `\n\n[…CipherScript: result truncated (${what}) so it can be pasted into ` +
  ...
  // function's real job is applying the .cs-tool-hide classes correctly.
  ...
  function findToolBlockSpot(item /*, chip */) {
    ...
    for (const mc of replies) {
      ...
      mc.querySelectorAll(S.codeWrap).forEach((cb) => {
        if (cb.closest(".cs-chip")) return;
        if (CMD_SHAPE.test(cb.textContent || "")) {
          cb.classList.add("cs-tool-hide");
          ...
          // rule `message-content.cs-cmd-mask code-block` keeps every
          // recreated block hidden with zero flash.
          mc.classList.add("cs-cmd-mask");
  ...
  return {
    ...
    unstableWarning:
      "Gemini tends to stop using the Unity tools after a while in long sessions (model behavior, not the extension). " +
