...
//    (b) a plain ``` fenced block (used for the ###CSHARP### execute_code form) is a
...
//    the framing is thin; the FULL CipherScript system prompt (with the "commands
...
const CSProvider = (() => {
  ...
  function textWithout(root, excludeSel) {
    if (!root) return "";
    const skip = ".cs-chip, " + S.reasoning + (excludeSel ? ", " + excludeSel : "");
    let t = "";
    const walk = (n) => {
  ...
  const editorEls = () => [...document.querySelectorAll(S.input)].filter((e) => !e.closest("#cs-root"));
  const isTextareaEditor = (e) => !!e && e.tagName === "TEXTAREA";
  // getEditor() = the ON-SCREEN editor (the Lexical div). The core anchors the
  // "Agent is working…" cover and the .cs-typing mask to P.getEditor(), so it MUST
  ...
  // #cs-bar into it. barAnchor() returns the rounded composer card; the core
  // keeps the bar in #cs-root and hugs the card's top edge.
  function barAnchor() {
  ...
  // contains the editor) so the cover leaves the controls - and the CS bar above -
  ...
  function streamText(item) {
    const b = bodyOf(item);
    return b ? textWithout(b, ".cs-chip") : "";
  }
  ...
  function readAssistant() {
    ...
    const b = bodyOf(item);
    return { present: true, reply: b ? textWithout(b, ".cs-chip").trim() : "", thinking: "", item };
  }
  ...
  const fileInputEl = () => {
    for (const inp of document.querySelectorAll('input[type="file"]')) {
      if (!inp.closest("#cs-root")) return inp;
    }
    return null;
  };
  ...
  function tagImages(images) {
    if (images && images.__csId == null) {
      try { Object.defineProperty(images, "__csId", { value: ++_imgSeq, enumerable: false }); }
      catch { images.__csId = ++_imgSeq; }
    }
    return images;
  }
  ...
  async function typeAndSend(text, images) {
    ...
    if (images && images.length && images !== _attachedImages) {
      ...
      try {
        ...
        diag("meta.tas.attached", { ok, imgId: images.__csId });
  ...
  // (its identity survives) with .cs-cmd-mask; the overlay.css rule keeps recreated
  ...
  return {
    ...
    // stay visible & usable - nor the CS bar anchored above. coverMaxH lifts the
    // core's 200px clamp so a grown (multi-line, up to the cs-typing 140px cap)
