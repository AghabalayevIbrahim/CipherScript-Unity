...
// Exports the same CSProvider interface as providers/deepseek.js; the core
...
//    CodeMirror/Monaco), wrapped in a <div class="not-prose">. CipherScript's
...
const CSProvider = (() => {
  ...
  function textWithout(root, excludeSel) {
    if (!root) return "";
    const skip = ".cs-chip" + (excludeSel ? ", " + excludeSel : "");
    let t = "";
    const walk = (n) => {
  ...
  // textareas are recaptcha/aria-hidden). Scope away CipherScript's own settings
  // textarea (#cs-root) so the send hooks' "not on a chat page" guard holds on
  // login/OAuth pages that have no site composer.
  const getEditor = () => {
    for (const e of document.querySelectorAll("form textarea")) {
      if (!e.closest("#cs-root")) return e;
    }
    return null;
  };
  const editorText = () => {
  ...
  // insert #cs-bar into it (the diff could nest the composer inside the bar, as
  // seen on Kimi/GLM). barAnchor() returns the rounded composer CARD; the core
  // (placeBar anchored branch) keeps the bar in #cs-root, hugs the card's top
  ...
  function setInputLock(on) {
    ...
    if (!ed) return;
    if (on) {
      if (!ed.dataset.csPlaceholder) ed.dataset.csPlaceholder = ed.getAttribute("placeholder") || "";
      ed.setAttribute("readonly", "");
      ed.setAttribute("placeholder", "⏳ Agent working… please wait");
    } else {
      ed.removeAttribute("readonly");
      if (ed.dataset.csPlaceholder != null) ed.setAttribute("placeholder", ed.dataset.csPlaceholder);
    }
  }
  ...
  function streamText(item) {
    if (!item) return "";
    const md = proseOf(item);
    return md ? textWithout(md, ".cs-chip") : "";
  }
  ...
  function readAssistant() {
    ...
    return {
      present: true,
      reply: md ? textWithout(md, ".cs-chip").trim() : "",
      thinking: "",
      item,
    };
  }
  ...
  function truncateForSend(text) {
    ...
    const marker =
      `\n\n[…CipherScript: result truncated to fit Arena's input limit - ` +
  ...
  async function typeAndSend(text, images) {
    ...
    diag("arena.tas.enter", {
      textLen: (text || "").length,
      imgId: images ? images.__csId : null,
      imgCount: images ? images.length : 0,
      attachedId: _attachedImages ? _attachedImages.__csId : null,
    ...
    if (images && images.length && images !== _attachedImages) {
      ...
      try {
        ...
        diag("arena.tas.attached", { imgId: images.__csId, ok, pendingAfter: pendingCount() });
      ...
      diag("arena.tas.skipAttach", { reason: !images || !images.length ? "no-images" : "same-set", imgId: images ? images.__csId : null });
    }
  ...
  // CipherScript only supports DIRECT. Battle / Side by Side force a fresh A/B
  ...
  // bounces the tab), which CipherScript can't drive. We nudge the mode dropdown
  ...
  // Visible mode guard for the CipherScript bar (core renderBar reads this every
  ...
  function modeWarning() {
    ...
    if (um)
      return `Turn off <b>${um.label}</b> (${um.tip} in the composer) - ` +
        `CipherScript only works in plain chat. ${um.label} mode uses a different ` +
    ...
    return `Switch the mode dropdown to <b>Direct</b> - CipherScript only works in ` +
  ...
  // CipherScript bar out of the way: the anchored bar is transparent but still a
  ...
  // up, the anchored CipherScript bar (a real, full-width element hugging the
  ...
  // with no open modal never matches. Our own UI lives in #cs-root and is skipped.
  function overlayBlocking() {
    for (const d of document.querySelectorAll('[role="dialog"]')) {
      if (d.closest("#cs-root")) continue;
  ...
  function fileFromImage(img, i) {
    ...
    return new File([arr], `cipherscript_${Date.now()}_${i}.${ext}`, { type: mime });
  }
  ...
  function tagImages(images) {
    if (images && images.__csId == null) {
      try { Object.defineProperty(images, "__csId", { value: ++_imgSeq, enumerable: false }); } catch { images.__csId = ++_imgSeq; }
    }
    return images;
  }
  ...
  // on the next send (wiping per-element .cs-tool-hide), so also mark the turn
  // (its identity survives - the chip is anchored at item level) with
  // .cs-cmd-mask; the overlay.css rule keeps recreated code wrappers hidden.
  const CMD_SHAPE = /"(?:command|tool)"\s*:\s*"|###\s*(?:c\s*#|csharp|cs)|###mcp_tool###/i;
  function findToolBlockSpot(item /*, chip */) {
    ...
    md.querySelectorAll(S.codeWrap).forEach((cw) => {
      if (cw.closest(".cs-chip")) return;
      if (CMD_SHAPE.test(cw.textContent || "")) {
        cw.classList.add("cs-tool-hide");
        item.classList.add("cs-cmd-mask");
    ...
    [...md.children].forEach((el) => {
      if (el.classList.contains("cs-chip") || el.querySelector(S.codeWrap)) return;
      const t = el.textContent || "";
      if (t.length < 600 && CMD_SHAPE.test(t)) {
        el.classList.add("cs-tool-hide");
        item.classList.add("cs-cmd-mask");
