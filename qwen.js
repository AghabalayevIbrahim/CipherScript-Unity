...
// Exports the same CSProvider interface as providers/deepseek.js and kimi.js;
...
const CSProvider = (() => {
  ...
  // `.view-line` only, e.g. a 50-line execute_code payload becomes just
  ...
  // MutationObserver snapshots its joined source into `pre.dataset.csCode`,
  ...
  function snapshotCode(pre) {
    ...
    if (!live) return;
    const prev = pre.dataset.csCode || "";
    if (live.length > prev.length) pre.dataset.csCode = live;
  }
  ...
  function codeText(pre) {
    const cached = pre.dataset.csCode || "";
    const live = codeLinesText(pre);
  ...
  function ensureCodeObserver() {
    ...
    // number of execute_code blocks. We time each pass and, once per second, log
  ...
  // `#cs-qwen-net` node as JSON { rid, text, done, t }. netLatest() reads it.
  ...
  function netLatest() {
    try {
      const n = document.getElementById("cs-qwen-net");
  ...
  // judged a still-streaming execute_code "done but unclosed" and fired a premature
  ...
  // Walk element text, skipping .cs-chip and any excluded selector.
  ...
  // Scope to the site's composer only; skip CipherScript's own injected textarea
  // (#cs-set-text inside #cs-root) so login pages without a site editor return
  ...
  // the positional index is NOT stable: two different turns (e.g. two execute_code
  ...
  // idx 10 shared by a 1384-char and a 775-char execute_code turn). The per-turn
  ...
  // keep #cs-bar in #cs-root (position:fixed) and hug it to the composer's top
  ...
  function setInputLock(on) {
    ...
    if (on) {
      ...
      ed.setAttribute("data-cs-locked", "1");
    } else {
      ed.removeAttribute("readonly");
      ed.removeAttribute("data-cs-locked");
  ...
  function streamText(item) {
    ...
    const bd = bodyEl(item);
    return bd ? textWithout(bd, ".cs-chip") : "";
  }
  ...
  function truncateForSend(text) {
    ...
    const marker =
      `\n\n[…CipherScript: result truncated to fit Qwen's ${SEND_CAP}-character input ` +
  ...
  async function typeAndSend(text, images) {
    ...
    const wasLocked = !!ed.getAttribute("data-cs-locked");
  ...
  // .cs-cmd-mask and let the CSS rule (overlay.css) re-hide recreated pre
  ...
  function findToolBlockSpot(item) {
    ...
    [...bd.children].forEach((el) => {
      if (el.classList.contains("cs-chip") || el.closest(S.codeWrap) || el.querySelector(S.codeWrap)) return;
  ...
  // build onto <html data-cs-qwen-ver>; read it from the page to confirm which
  ...
  function setVersionBeacon() {
    try { document.documentElement.setAttribute("data-cs-qwen-ver", QWEN_VER); } catch {}
  }
  ...
  // tool-calls instead of CipherScript commands. Removed 2026-07: the real cause of
  ...
  const MODEL_VIS_LS = "csQwenModelVision2"; // bumped: old key may hold a stale Max-Preview=false
